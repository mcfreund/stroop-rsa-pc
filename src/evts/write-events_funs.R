...
write.event <- function(
  ...
  names.expected <- c(name.var, name.onset, "subj", "wave", "session", "task", "run")
  names.ok <- all(names.expected %in% names(d))
  if (!names.ok) stop("d missing cols: name.var | name.onset | subj | session | task | wave")
  ...
  if (length(c(subj, wave, sess, task, var.level)) != 5) stop(">1 subj | wave | session | task | var.level")
  ...
  if (!wave %in% c("wave1", "wave2", "wave3")) stop("wave name error")
  
  ## create path / directory
  
  dir.input <- file.path(dir.analysis, subj, wave, subdir.input.data, task, sess)
  ...
  fname <- file.path(dir.input, paste0(subj, "_", wave, "_", task, "_", sess, "_", var.level, fname.suffix))
...
write.events <- function(.d, .args) {
  ...
  if (length(c(subj, sess, task, wave)) != 4) stop(">1 subj | session | task | wave")
  if (!sess %in% c("baseline", "proactive", "reactive")) stop("session name error")
  if (!task %in% c("Axcpt", "Cuedts", "Stern", "Stroop")) stop("task name error")
  if (!wave %in% c("wave1", "wave2", "wave3")) stop("wave name error")

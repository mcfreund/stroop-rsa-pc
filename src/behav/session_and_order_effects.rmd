---
title: "behavioral analysis: session and order effects"
output: 
    html_document:
        toc: true
        toc_depth: 3
...
- MCMI, N=`r length(subjects_mcmi)`: unrelated subjects with baseline (MC) wave 1, proactive (MI) wave 1
...
drt <- filter(drt, rt > 0 & rt < 2000)

drt_mc1mi1 <- drt %>% filter(wave == "wave1" & subj %in% subjects_mcmi & session %in% c("baseline", "proactive"))

drt_mi1mc2 <- rbind(
    drt %>% filter(wave == "wave2" & subj %in% subjects_mimc & session == "baseline"),
    drt %>% filter(wave == "wave1" & subj %in% subjects_mimc & session == "proactive")
)

drt_wave1 <- drt %>% filter(wave == "wave1" & subj %in% subjects_wave1)
...
### baseline wave 1 (MC1) versus proactive wave 1 (MI1): MCMI
...
drt_mc1mi1 %>%
    ...
    stat_summary(
        fun.data = mean_cl_boot, position = position_dodge(width = 0.1), geom = "errorbar", size = 1.5, width = 0
        ) +
    stat_summary(fun = mean, position = position_dodge(width = 0.1), geom = "point", size = 3)

ebm_mcmi <- lmer(rt ~ trial_type * session + (trial_type * session | subj), drt_mc1mi1, subset = pc != "pc50")
...
### baseline wave 2 (MC2) versus proactive wave 1 (MI1): MIMC
...
ebm_mimc <- lmer(rt ~ trial_type * pc + (trial_type * pc | subj), drt_mi1mc2, subset = pc != "pc50")
...
drt_wave1 %>%
    filter(pc != "pc50" & session %in% c("reactive", "proactive")) %>%
    ggplot(aes(trial_type, rt, color = session)) +
    stat_summary(
        fun.data = mean_cl_boot, position = position_dodge(width = 0.1), geom = "errorbar", size = 1.5, width = 0
        ) +
...
ebm_wave1 <- 
    ...
    drt_wave1, 
    subset = pc != "pc50" & session %in% c("reactive", "proactive")
    )
...
### baseline wave 1 (MC1) versus proactive wave2 (MI2): MCMI
...
ebm_mcmi_pc50 <- lmer(rt ~ trial_type * session + (trial_type * session | subj), drt_mc1mi1, subset = pc == "pc50")
...
ebm_mimc_pc50 <- lmer(rt ~ trial_type * session + (trial_type * session | subj), drt_mi1mc2, subset = pc == "pc50")

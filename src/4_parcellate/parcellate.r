...
for (result_i in seq_along(dir_results)) {
    ...
    files <- file.path(dir_result, paste0(file_prefix, "_", input[result_i]$subj, "_", runs[run_i], "_", hemis, "_", file_suffix))  ## L then R

    l <- list(run1 = NULL, run2 = NULL)
    for (run_i in seq_along(runs)) {

        files <- file.path(dir_result, paste0(file_prefix, "_", input[result_i]$subj, "_", runs[run_i], "_", hemis, "_", file_suffix))  ## L then R
        if (any(!file.exists(files))) stop(paste0("file does not exist in dir: ", dir_result))

        gii <- lapply(files, read_gifti)
        
        l[[run_i]] <- parcellated_image(gii, pattern = "_Coef")  ## construct parcellated image

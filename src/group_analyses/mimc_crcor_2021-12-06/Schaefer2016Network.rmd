---
title: "MI wave 1 vs MC wave 2 :: ttypes: `r params$ttype_subset`, prewh: `r params$prewh`"
output: 
    html_document:
        toc: true
        toc_depth: 2
...
#render_report(name = "Schaefer2016Network", src_dir = "group_analyses/mimc_crcor_2021-12-06", params = list(ttype_subset = "bias", prewh = "none"))
#render_report(name = "Schaefer2016Network", src_dir = "group_analyses/mimc_crcor_2021-12-06", params = list(ttype_subset = "pc50", prewh = "none"))
library(knitr)
library(dplyr)
library(tidyr)
library(ggplot2)
...
suffices <- c("proactive_wave1", "baseline_wave2")

## read subject RDMs:
rdm <- enlist(suffices)
for (suffix in suffices) {
    session_wave <- strsplit(suffix, "_")[[1]]
    rdm[[suffix]] <- 
        read_rdms(
            .measure = measure, .prewh = prewh,
            .glmname = glmname, .roiset = roiset, .waves = session_wave[2],
            .session = session_wave[1], .subjects = subjects,
            .ttype_subset = ttype_subset,
            .ttypes1 = intersect(ttypes_by_run[[session_wave[1]]]$run1, ttypes[[ttype_subset]]),
            .ttypes2 = intersect(ttypes_by_run[[session_wave[1]]]$run2, ttypes[[ttype_subset]]),
...
for (suffix in suffices) {
    file_name <- 
        construct_filename_weights(
            ...
            roiset = roiset, prewh = prewh, suffix = paste0("__seswave-", suffix)
        )
    dat[[suffix]] <- fread(file_name)
}
dat <- rbindlist(dat)
...
dat_contr <- dat %>% 
    pivot_wider(id_cols = c(subject, term, roi), names_from = "session", values_from = "b") %>%
...
for (session in sessions) {
    ...
    for (mod in models$crcor) {
        rdm_models <- 
            read_model_rdm(
                model = mod, measure_type = "similarity", session = session, ttype_subset = ttype_subset
            )
        p <- rdm_models %>% 
            melt_mat %>% 
...
for (suffix in suffices) {
    ...
    p <- rdm[[suffix]] %>%
        ...
        labs(title = paste0("Group-mean matrix: ", suffix), x = "Run 2", y = "Run 1")
...
for (suffix in suffices) {
    ...
    p <- rdm[[suffix]][, , , 1:5, ] %>%
        ...
        theme(axis.text.x = element_text(angle = 90, hjust = 0), legend.position = "top") +
        labs(title = paste0("Single-subject matrix: ", suffix), x = "Run 2", y = "Run 1")
...
dat %>%
    ...
    filter(term %in% c("conjunction", "distractor", "incongruency", "target")) %>%
